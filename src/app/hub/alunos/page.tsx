...
export default function StudentsDirectoryPage() {
  ...
  return <main className="mx-auto w-full max-w-7xl p-4 py-8 sm:p-6 lg:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px]">
    <header className="mb-7 flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-sky-700 dark:text-sky-300">Diretório escolar</p><h1 className="mt-2 text-3xl font-black tracking-tight">Estudantes</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Localize estudantes, consulte o histórico pedagógico e registre ocorrências em um único prontuário.</p></div>{data.source ? <p className="rounded-xl border bg-white px-4 py-2 text-sm text-muted-foreground shadow-sm dark:bg-slate-900">Contexto atual: <strong className="text-foreground">{data.source.term}º bimestre de {data.source.schoolYear}</strong></p> : null}</header>
    ...
    <section className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3 min-[1800px]:grid-cols-4 min-[2400px]:grid-cols-5">{filteredStudents.slice(0, visibleCount).map((student) => <Link key={student.studentId} href={`/hub/alunos/${student.studentId}`} className="group rounded-2xl border bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:shadow-md dark:bg-slate-900 dark:hover:border-sky-800"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="block truncate text-sm">{student.name}</strong><p className="mt-1 text-sm text-muted-foreground">Matrícula {student.enrollmentNumber}</p></div><ArrowUpRight className="mt-0.5 size-4 shrink-0 text-slate-400 transition group-hover:translate-x-0.5 group-hover:text-sky-600" /></div>{student.current ? <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-sm text-muted-foreground"><span>Turma <strong className="text-foreground">{student.current.className}</strong></span><span>{student.current.gradeLevel ? `${student.current.gradeLevel}ª série` : "Série não identificada"}</span><span>Frequência {student.current.attendanceRate === null ? "—" : `${student.current.attendanceRate.toLocaleString("pt-BR")}%`}</span></div> : <p className="mt-3 text-sm text-muted-foreground">Fora do relatório mais recente.</p>}{student.situation !== "regular" ? <p className={`mt-2 text-sm ${situationClassName(student.situation)}`}>{STUDENT_SITUATION_LABELS[student.situation]}{student.situation === "transferred" ? " · fora do fluxo" : ""}</p> : null}{student.occurrences.latest ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50/70 px-3 py-2.5 text-sm dark:border-amber-900 dark:bg-amber-950/25"><div className="flex items-center justify-between gap-2"><span className="font-semibold text-amber-900 dark:text-amber-200">{student.occurrences.count} {student.occurrences.count === 1 ? "ocorrência" : "ocorrências"}</span><span className="text-amber-700 dark:text-amber-300">{formatDate(student.occurrences.latest.occurredOn)}</span></div><p className="mt-1 truncate text-amber-800/80 dark:text-amber-200/70">Última: {STUDENT_OCCURRENCE_LABELS[student.occurrences.latest.category]}</p></div> : null}</Link>)}</section>
...
function Metric({ icon: Icon, value, label }: { icon: typeof UsersRound; value: number; label: string }) {
  return <div className="rounded-2xl border bg-white p-4 shadow-sm dark:bg-slate-900"><div className="flex items-center gap-3"><span className="grid size-9 place-items-center rounded-xl bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"><Icon className="size-4" /></span><div><strong className="block text-xl font-black">{value.toLocaleString("pt-BR")}</strong><span className="text-sm text-muted-foreground">{label}</span></div></div></div>;
}
