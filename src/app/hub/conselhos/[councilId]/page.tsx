...
import { AlertCircle, ArrowLeft, BookOpenCheck, CalendarDays, CheckCircle2, ChevronRight, Clock3, Download, GraduationCap, Loader2, Search, Trash2, TrendingDown, Upload, Users } from "lucide-react";
...
type Overview = {
  ...
  classes: Array<{ id: string; official_code: string; display_name: string; status: string; studentCount?: number; alertCount?: number }>;
  metrics: { students: number; academicAlerts: number; lowAttendance: number; worsened: number; pendingInterventions: number };
  studentDetails: StudentDetail[];
  interventionDetails: InterventionDetail[];
  subjectRanking: Array<{ name: string; low: number; numeric: number; percentage: number }>;
};

type MetricKey = "students" | "academicAlerts" | "lowAttendance" | "worsened" | "pendingInterventions";
...
const metricStyles: Record<MetricKey, { card: string; icon: string }> = {
  ...
  academicAlerts: { card: "border-blue-200/70 bg-blue-50/60 hover:bg-blue-50 dark:border-blue-900 dark:bg-blue-950/20", icon: "text-blue-600 dark:text-blue-400" },
...
export default function CouncilDashboardPage() {
  ...
  const detailedStudents = useMemo(() => {
    ...
    const selected = data.studentDetails.filter((student) => {
      if (activeMetric === "academicAlerts") return student.alerts.academicAlert;
    ...
    return [...filtered].sort((a, b) => {
      if (activeMetric === "academicAlerts" && a.alerts.currentLowGradeCount !== b.alerts.currentLowGradeCount) return b.alerts.currentLowGradeCount - a.alerts.currentLowGradeCount;
  ...
  const metricCards: Array<{ key: MetricKey; label: string; value: number; icon: typeof Users; description: string }> = [
    ...
    { key: "academicAlerts", label: "Alerta acadêmico", value: data.metrics.academicAlerts, icon: GraduationCap, description: "Quatro ou mais disciplinas com nota abaixo de 6,0" },
  ...
  return <main className="mx-auto max-w-7xl p-4 py-8 sm:p-8">
    ...
    <section className="mb-6 rounded-2xl border bg-blue-50/60 p-5 dark:bg-blue-950/20"><div className="flex gap-3"><BookOpenCheck className="mt-0.5 h-5 w-5 text-blue-700" /><div><h2 className="font-semibold">Critérios deste conselho</h2><p className="mt-1 text-sm text-muted-foreground">Nota numérica abaixo de 6,0; alerta acadêmico com 4 ou mais disciplinas; frequência anual abaixo de 80%; piora quando aumenta a quantidade de disciplinas com nota baixa em relação ao bimestre anterior disponível. Marcadores nunca contam como zero.</p></div></div></section>
