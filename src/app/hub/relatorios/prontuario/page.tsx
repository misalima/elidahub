...
function StudentRecordDocument({ data, includeHistory, includeInterventions, includeOccurrences, position, total }: { data: StudentProfileData; includeHistory: boolean; includeInterventions: boolean; includeOccurrences: boolean; position: number; total: number }) {
  return <article className="record-student-document bg-white p-[12mm] shadow-lg print:p-0">
    <header className="mb-5 flex items-center gap-4 border-b-2 border-slate-900 pb-3"><Image src="/logo_escola.png" alt={`Logo da ${SCHOOL_NAME}`} width={56} height={56} className="size-14 object-contain" unoptimized /><div className="min-w-0 flex-1"><p className="text-sm font-bold uppercase tracking-wide">{SCHOOL_NAME}</p><p className="text-sm text-slate-600">{SCHOOL_LOCATION}</p><h1 className="mt-1.5 text-lg font-black">Prontuário individual do estudante</h1></div>{total > 1 ? <span className="text-sm text-slate-500">{position}/{total}</span> : null}</header>
    <section className="record-avoid-break rounded-lg border border-slate-300 p-3"><div className="flex items-start justify-between gap-4"><div><h2 className="text-base font-black">{data.student.name}</h2><p className="mt-0.5 text-sm text-slate-600">Matrícula {data.student.enrollmentNumber}{data.latest ? ` · Turma ${data.latest.class.name}` : ""}</p></div><span className="rounded border border-slate-400 px-2 py-1 text-sm font-bold uppercase">{STUDENT_SITUATION_LABELS[data.student.currentSituation]}</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-5"><RecordMetric value={data.summary.councils} label="Conselhos" /><RecordMetric value={data.latest?.snapshot.attendanceRate === null || !data.latest ? "—" : `${data.latest.snapshot.attendanceRate}%`} label="Frequência atual" /><RecordMetric value={data.summary.behaviorRecords} label="Comportamentos" /><RecordMetric value={data.summary.openInterventions} label="Intervenções abertas" /><RecordMetric value={data.summary.occurrences} label="Ocorrências" /></div></section>
    ...
    <footer className="mt-7 border-t border-slate-300 pt-2 text-sm text-slate-500">Documento gerado pelo {HUB_NAME} em {new Date().toLocaleString("pt-BR")}.</footer>
  </article>;
}

function HistorySection({ data }: { data: StudentProfileData }) {
  return <section className="mt-6"><SectionTitle>Histórico dos Conselhos de Classe</SectionTitle>{data.history.length ? <div className="mt-3 space-y-3">{data.history.map((item) => <article key={`${item.council.id}:${item.enrollment.id}`} className="rounded-lg border border-slate-300 p-3"><div className="record-avoid-break"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-bold">{item.council.term}º bimestre de {item.council.schoolYear} · Turma {item.class.name}</h3><p className="mt-0.5 text-sm text-slate-500">Conselho realizado em {formatDate(item.council.meetingDate)} · {item.class.officialCode}</p></div><span className="text-sm font-bold text-slate-700">{academicLabels[item.alerts.academicStatus]}</span></div><div className="mt-2 grid gap-1.5 text-sm sm:grid-cols-3"><p><strong>Frequência:</strong> {item.snapshot.attendanceRate === null ? "não informada" : `${item.snapshot.attendanceRate}%`}</p><p><strong>Fluxo:</strong> {projectionLabels[item.projection.status]}</p><p><strong>Discutido:</strong> {item.enrollment.discussed ? "sim" : "não"}</p></div>{item.alerts.reasons.length ? <p className="mt-2 text-sm"><strong>Alertas:</strong> {item.alerts.reasons.join(" · ")}</p> : null}{item.enrollment.pedagogicalObservation ? <p className="mt-1.5 text-sm"><strong>Observação pedagógica:</strong> {item.enrollment.pedagogicalObservation}</p> : null}{item.enrollment.positiveNotes ? <p className="mt-1.5 text-sm"><strong>Pontos positivos:</strong> {item.enrollment.positiveNotes}</p> : null}{item.behaviors.length ? <p className="mt-1.5 text-sm"><strong>Comportamento:</strong> {item.behaviors.map((behavior) => `${BEHAVIOR_LABELS[behavior.category]}${behavior.description ? ` — ${behavior.description}` : ""}`).join(" · ")}</p> : null}</div><SubjectPerformanceTable item={item} /></article>)}</div> : <p className="mt-3 text-sm italic text-slate-500">Nenhum Conselho confirmado no histórico.</p>}</section>;
}
...
function InterventionsSection({ data }: { data: StudentProfileData }) {
  const originLabel = (item: StudentProfileData["interventions"][number]) => item.sourceType === "class_council"
    ? `${item.origin.term}º bimestre/${item.origin.schoolYear} · ${item.origin.className}`
    : item.sourceType === "student_profile"
      ? `Prontuário do estudante${item.targetClass?.className ? ` · ${item.targetClass.className}` : ""}`
      : `Central de intervenções${item.targetClass?.className ? ` · ${item.targetClass.className}` : ""}`;
  return <section className="mt-6"><SectionTitle>Intervenções</SectionTitle>{data.interventions.length ? <table className="mt-2 w-full table-fixed border-collapse text-sm"><colgroup><col className="w-[43%]" /><col className="w-[18%]" /><col className="w-[15%]" /><col className="w-[24%]" /></colgroup><thead><tr className="border-b border-slate-500 text-left"><th className="p-1">Intervenção</th><th className="p-1">Responsável</th><th className="p-1">Prazo</th><th className="p-1">Status/retorno</th></tr></thead><tbody>{data.interventions.map((item) => <tr key={item.id} className="record-avoid-break border-b border-slate-200 align-top"><td className="p-1"><strong>{item.description}</strong><p className="mt-0.5 text-sm text-slate-500">{originLabel(item)}</p>{item.reason ? <p className="mt-0.5 text-sm"><strong>Motivo:</strong> {item.reason}</p> : null}</td><td className="p-1">{item.responsibleName ?? "—"}</td><td className="p-1">{formatDate(item.dueDate)}</td><td className="p-1"><strong>{interventionLabels[item.status]}</strong>{item.outcome ? <p className="mt-0.5">{item.outcome}</p> : item.cancellationReason ? <p className="mt-0.5">{item.cancellationReason}</p> : null}</td></tr>)}</tbody></table> : <p className="mt-3 text-sm italic text-slate-500">Nenhuma intervenção registrada.</p>}</section>;
}

function OccurrencesSection({ data }: { data: StudentProfileData }) {
  return <section className="mt-6"><SectionTitle>Ocorrências</SectionTitle>{data.occurrences.length ? <div className="mt-2 divide-y rounded-lg border border-slate-300">{data.occurrences.map((item) => <article key={item.id} className="record-avoid-break p-2.5"><div className="flex items-start justify-between gap-3"><strong className="text-sm">{STUDENT_OCCURRENCE_LABELS[item.category]}{item.targetType === "collective" ? ` · Coletiva com ${item.participants.length} estudantes` : ""}</strong><span className="text-sm text-slate-500">{formatDate(item.occurredOn)} · {item.guardianNotified ? "responsável ciente" : "responsável não cientificado"}</span></div>{item.targetType === "collective" ? <p className="mt-1 text-sm text-slate-500">Envolvidos: {item.participants.map((participant) => participant.name).join(", ")}</p> : null}{item.notes ? <p className="mt-1 text-sm leading-4">{item.notes}</p> : null}</article>)}</div> : <p className="mt-3 text-sm italic text-slate-500">Nenhuma ocorrência registrada.</p>}</section>;
}
...
function SectionTitle({ children }: { children: React.ReactNode }) { return <h2 className="border-b border-slate-400 pb-1.5 text-sm font-black uppercase tracking-wide">{children}</h2>; }
function RecordMetric({ value, label }: { value: string | number; label: string }) { return <div className="rounded border border-slate-200 bg-slate-50 px-2 py-1.5"><strong className="block text-sm">{value}</strong><span className="text-sm text-slate-500">{label}</span></div>; }
