...
function MissingGradesDocument({ data }: { data: PedagogicalDashboardOverviewData }) {
  ...
  return <>
    <header className="council-print-header mb-5 flex items-center gap-4 border-b-2 border-slate-900 pb-3">
      ...
      <div><p className="text-sm font-bold uppercase tracking-wide">{SCHOOL_NAME}</p><p className="text-sm text-slate-600">{SCHOOL_LOCATION}</p><h1 className="mt-1.5 text-lg font-black">Relatório de notas pendentes</h1><p className="text-sm text-slate-600">{selected.year} · até o {selected.term}º bimestre · relatório gerado em {sourceDate(data.source?.generatedAt)}</p></div>
    </header>
    ...
    {groups.length ? <div className="mt-5 space-y-5">{groups.map((group) => {
      ...
      return <section key={group.classId} className="missing-grades-group">
        <div className="missing-grades-group-header flex items-end justify-between gap-4 border-b-2 border-slate-800 pb-1.5"><div><h2 className="text-sm font-black">Turma {group.className}</h2><p className="text-sm text-slate-500">{group.classCode}</p></div><strong className="text-sm">{classTotal} {classTotal === 1 ? "nota pendente" : "notas pendentes"}</strong></div>
        <table className="mt-1.5 w-full table-fixed border-collapse text-sm leading-snug">
    ...
    })}</div> : <div className="mt-8 rounded-xl border border-emerald-300 bg-emerald-50 p-8 text-center text-emerald-900"><CheckCircle2 className="mx-auto size-7" /><h2 className="mt-3 font-bold">Nenhuma nota pendente</h2><p className="mt-1 text-sm">A versão selecionada não possui notas faltantes nos bimestres transcorridos.</p></div>}

    <p className="mt-7 border-t border-slate-300 pt-2 text-sm text-slate-500">Fonte: versão {data.source?.version ?? "—"} do Relatório de Desempenho. Documento gerado pelo {HUB_NAME}.</p>
    <style jsx global>{`
...
function Summary({ value, label }: { value: number; label: string }) {
  return <div className="print-avoid-break rounded-lg border border-slate-300 px-3 py-2"><strong className="block text-lg font-black">{value.toLocaleString("pt-BR")}</strong><span className="text-sm text-slate-600">{label}</span></div>;
}
