...
export function CouncilGeneralPrintDocument({ data, compact = false }: { data: CouncilPrintOverviewData; compact?: boolean }) {
  ...
  return <>
    ...
    <section className="grid grid-cols-3 gap-2 sm:grid-cols-5">
      <Metric value={data.metrics.students} label="Estudantes" /><Metric value={data.metrics.atRisk} label="Em risco" /><Metric value={data.metrics.lowAttendance} label="Frequência abaixo de 80%" /><Metric value={data.metrics.worsened} label="Pioraram" /><Metric value={data.metrics.pendingInterventions} label="Intervenções pendentes" />
    </section>
    ...
    <p className="mt-8 border-t border-slate-300 pt-2 text-[10px] text-slate-500">Documento gerado pelo Felix Hub. Raça/cor é omitida desta impressão.</p>
  </>;
}
...
export function CouncilClassPrintDocument({ data }: { data: ClassWorkspaceData }) {
  ...
  return <>
    ...
    <p className="mt-8 border-t border-slate-300 pt-2 text-[10px] text-slate-500">Documento gerado pelo Felix Hub. Raça/cor é omitida desta impressão; a indicação PCD é preservada junto ao estudante.</p>
  </>;
}
