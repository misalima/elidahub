...
import { HUB_NAME, SCHOOL_LOCATION, SCHOOL_NAME } from "@/constants/main/school";
...
export function InterventionWorkspace({ readOnly = false, initialFilters, backHref = "/hub" }: { readOnly?: boolean; initialFilters?: InterventionWorkspaceFilters; backHref?: string }) {
  ...
  return <main className={`intervention-report-root mx-auto w-full max-w-7xl p-4 py-8 sm:p-6 lg:p-8 min-[1800px]:max-w-[1600px] min-[2400px]:max-w-[1800px] ${mode === "compact" ? "report-compact" : "report-follow-up"}`}>
    <div className="report-screen-only"><div className="mb-5 flex items-center justify-between gap-3"><Link href={backHref} className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground transition hover:text-foreground"><ArrowLeft className="size-4" />{readOnly ? "Voltar aos relatórios" : "Voltar ao painel"}</Link><div className="flex items-center gap-2">{isFetching ? <span role="status" className="flex items-center gap-1.5 text-sm font-medium text-primary"><Loader2 className="size-3.5 animate-spin" />Atualizando</span> : null}{!readOnly ? <><Button asChild type="button" variant="outline" className="rounded-xl"><Link href={reportHref}><Printer className="size-4" />Abrir relatório</Link></Button><Button type="button" className="rounded-xl" onClick={() => setCreating(true)}><Plus className="size-4" />Nova intervenção</Button></> : null}</div></div>
      <header className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-sm font-bold uppercase tracking-[0.18em] text-primary">{readOnly ? "Pré-visualização do documento" : "Acompanhamento operacional"}</p><h1 className="mt-2 text-3xl font-black tracking-tight">{readOnly ? "Relatório de intervenções" : "Intervenções"}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">{readOnly ? "Confira o recorte selecionado e escolha o formato antes de imprimir ou salvar em PDF." : "Acompanhe em um só lugar as ações definidas nos Conselhos e as registradas diretamente para os estudantes."}</p></div>{readOnly ? <div className="flex flex-wrap gap-2"><div className="flex rounded-xl border bg-white p-1 shadow-sm dark:bg-slate-900"><button type="button" onClick={() => setMode("follow_up")} className={`rounded-lg px-3 py-2 text-sm font-bold transition ${mode === "follow_up" ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "text-muted-foreground"}`}>Acompanhamento</button><button type="button" onClick={() => setMode("compact")} className={`rounded-lg px-3 py-2 text-sm font-bold transition ${mode === "compact" ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "text-muted-foreground"}`}>Compacto</button></div><button type="button" role="switch" aria-checked={showReasons} onClick={() => setShowReasons((current) => !current)} className="flex h-10 cursor-pointer items-center gap-2 rounded-xl border bg-white px-3 text-sm font-bold text-slate-700 shadow-sm outline-none transition hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-ring dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800"><span aria-hidden="true" className={`relative h-5 w-9 rounded-full transition-colors ${showReasons ? "bg-primary" : "bg-slate-300 dark:bg-slate-700"}`}><span className={`absolute left-0.5 top-0.5 size-4 rounded-full bg-white shadow-sm transition-transform ${showReasons ? "translate-x-4" : "translate-x-0"}`} /></span>Mostrar motivos</button><Button className="rounded-xl" onClick={() => window.print()}><Printer className="size-4" />Imprimir ou salvar em PDF</Button></div> : null}</header>
    ...
    <article className={readOnly ? "intervention-print-document mt-6 bg-white text-slate-950 shadow-sm print:mt-0 print:shadow-none" : "intervention-operational-list mt-6"}>
      ...
      {readOnly ? <footer className="mt-8 border-t border-slate-300 pt-2 text-[9px] text-slate-500">Documento operacional gerado pelo {HUB_NAME}. A origem identifica onde a ação foi definida; o status reflete o acompanhamento atual.</footer> : null}
    </article>
...
function Metric({ icon: Icon, value, label, alert = false }: { icon: typeof ClipboardCheck; value: number; label: string; alert?: boolean }) {
  return <div className="rounded-2xl border bg-white p-4 shadow-sm dark:bg-slate-900"><div className="flex items-center gap-3"><span className={`grid size-9 place-items-center rounded-xl ${alert ? "bg-rose-50 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}><Icon className="size-4" /></span><div><strong className="block text-xl font-black">{value.toLocaleString("pt-BR")}</strong><span className="text-sm text-muted-foreground">{label}</span></div></div></div>;
}
...
function CentralInterventionCreator({ open, onOpenChange, onCreated }: { open: boolean; onOpenChange: (open: boolean) => void; onCreated: () => void }) {
  ...
  const canSubmit = Boolean(studentId && description.trim() && reason.trim());
  ...
  return <Dialog open={open} onOpenChange={(next) => { if (!saving) onOpenChange(next); }}>
    <DialogContent className="max-h-[90vh] overflow-hidden rounded-2xl p-0 sm:max-w-2xl">
      <form onSubmit={submit} className="flex max-h-[90vh] min-h-0 flex-col">
        <DialogHeader className="shrink-0 border-b px-6 pb-4 pt-5 pr-14 text-left">
          <div className="flex items-start gap-3">
            <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><ClipboardCheck className="size-5" /></span>
            <div>
              <DialogTitle className="text-xl">Nova intervenção</DialogTitle>
              <DialogDescription className="mt-1 max-w-xl leading-6">Registre uma ação individual. Ela também ficará disponível no prontuário do estudante.</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2 sm:col-span-2">
              <Label htmlFor="central-intervention-student">Estudante <span className="text-destructive" aria-hidden="true">*</span></Label>
              {selectedStudent ? <div className="flex items-center justify-between gap-3 rounded-xl border border-primary/25 bg-primary/5 px-3.5 py-3">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="grid size-9 shrink-0 place-items-center rounded-full bg-white text-primary shadow-sm dark:bg-slate-900"><UserRound className="size-4" /></span>
                  <div className="min-w-0"><strong className="block truncate text-sm">{selectedStudent.name}</strong><span className="block truncate text-sm text-muted-foreground">Matrícula {selectedStudent.enrollmentNumber}{selectedStudent.current ? ` · Turma ${selectedStudent.current.className}` : " · sem turma atual"}</span></div>
                </div>
                <Button type="button" variant="ghost" size="sm" className="shrink-0" onClick={() => setStudentId("")}>Trocar</Button>
              </div> : <>
                <div className="relative">
                  <Search className="pointer-events-none absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" />
                  <Input id="central-intervention-student" type="search" value={studentSearch} onChange={(event) => setStudentSearch(event.target.value)} className="h-11 rounded-xl pl-10 pr-10" placeholder="Nome, matrícula ou turma" autoComplete="off" />
                  {studentsQuery.isPending ? <Loader2 aria-label="Carregando estudantes" className="absolute right-3.5 top-1/2 size-4 -translate-y-1/2 animate-spin text-primary" /> : null}
                </div>
                <div aria-live="polite">
                  {!studentsQuery.isPending && normalizedSearch.length < 2 ? <p className="text-sm text-muted-foreground">Digite ao menos 2 caracteres para buscar.</p> : null}
                  {!studentsQuery.isPending && normalizedSearch.length >= 2 ? <div className="max-h-44 overflow-y-auto rounded-xl border bg-background shadow-sm">
                    {results.length ? results.map((student) => <button key={student.studentId} type="button" onClick={() => { setStudentId(student.studentId); setStudentSearch(""); }} className="flex w-full items-center justify-between gap-3 border-b px-3.5 py-2.5 text-left transition last:border-0 hover:bg-primary/5 focus-visible:bg-primary/5 focus-visible:outline-none">
                      <span className="min-w-0"><strong className="block truncate text-sm">{student.name}</strong><span className="block truncate text-sm text-muted-foreground">Matrícula {student.enrollmentNumber}</span></span>
                      <span className="shrink-0 text-sm text-muted-foreground">{student.current?.className ? `Turma ${student.current.className}` : "Sem turma"}</span>
                    </button>) : <p className="p-4 text-center text-sm text-muted-foreground">Nenhum estudante encontrado.</p>}
                  </div> : null}
                </div>
              </>}
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="central-intervention-action">Ação/intervenção <span className="text-destructive" aria-hidden="true">*</span></Label>
              <Textarea id="central-intervention-action" value={description} onChange={(event) => setDescription(event.target.value)} rows={4} maxLength={2000} className="min-h-24 resize-y" placeholder="O que será feito para apoiar o estudante?" required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="central-intervention-reason">Motivo ou contexto <span className="text-destructive" aria-hidden="true">*</span></Label>
              <Textarea id="central-intervention-reason" value={reason} onChange={(event) => setReason(event.target.value)} rows={4} maxLength={2000} className="min-h-24 resize-y" placeholder="O que motivou este acompanhamento?" required />
            </div>

            <div className="space-y-2">
              <Label htmlFor="central-intervention-responsible">Responsável <span className="font-normal text-muted-foreground">(opcional)</span></Label>
              <Input id="central-intervention-responsible" value={responsibleName} onChange={(event) => setResponsibleName(event.target.value)} maxLength={200} placeholder="Pessoa ou equipe responsável" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="central-intervention-due-date">Prazo <span className="font-normal text-muted-foreground">(opcional)</span></Label>
              <Input id="central-intervention-due-date" type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} />
            </div>
          </div>
        </div>

        <DialogFooter className="shrink-0 border-t bg-slate-50/70 px-6 py-4 dark:bg-slate-900/50 sm:items-center sm:justify-between">
          <p className="text-sm text-muted-foreground"><span className="text-destructive">*</span> Campos obrigatórios</p>
          <div className="flex flex-col-reverse gap-2 sm:flex-row">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={saving}>Cancelar</Button>
            <Button type="submit" disabled={saving || !canSubmit}>{saving ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />}{saving ? "Criando..." : "Criar intervenção"}</Button>
          </div>
        </DialogFooter>
      </form>
    </DialogContent>
